...
2. https://chatgpt.com/codex/settings/environments → new environment: image universal; `CODEX_ENV_RUST_VERSION=1.95.0`, `CODEX_ENV_PYTHON_VERSION=3.12`; setup script `bash scripts/cloud-setup.sh`; maintenance script `cargo fetch --locked`; agent-phase internet `Common dependencies`; no variables or secrets.
