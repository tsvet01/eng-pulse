...
use crate::manifest::{gcs_object_path, ManifestEntry};
...
pub(crate) fn has_both_polarities(feedback: &[FeedbackEntry]) -> bool {
    let has_up = feedback
        .iter()
        .any(|f| f.effective_feedback() == Some("up"));
    let has_down = feedback
        .iter()
        .any(|f| f.effective_feedback() == Some("down"));
    has_up && has_down
}
...
pub(crate) async fn download_feedback_excerpts(
    ...
    for entry in entries {
            ...
            .await
        {
            ...
            Err(e) => {
                warn!(url = %entry.summary_url, error = %e, "Failed to download feedback summary")
            }
        }
    }
    results
...
pub(crate) async fn build_calibration_context(
    ...
    let ups: Vec<&FeedbackEntry> = feedback
        .iter()
        .filter(|f| f.effective_feedback() == Some("up"))
        .take(2)
        .collect();
    let downs: Vec<&FeedbackEntry> = feedback
        .iter()
        .filter(|f| f.effective_feedback() == Some("down"))
        .take(2)
        .collect();
    ...
    info!(
        "Built calibration context with {} highly and {} poorly rated examples",
        highly_rated.len(),
        poorly_rated.len()
    );
    Some(context)
